...
export default function RISCWorkload() {
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        parsed.push({
          member,
          loanType: rawType,
          group: classifyLoan(rawType),
          fundDate,
          fundDateKey: statusOpen ? (fundDate ? dateKey(fundDate) : null) : null,
          checkInDate,
          checkInDateKey: checkInDate ? dateKey(checkInDate) : null,
          pmOpen,
          statusOpen,
          status,
          borrower,
    ...
    return (
      <div style={styles.container}>
        <div style={styles.header}>
          <h1 style={styles.title}>RISC Team Workload</h1>
          <p style={styles.subtitle}>Upload the archive file to view assignments</p>
        </div>
        <div
  ...
  const memberData = MEMBERS.map((m) => {
    ...
    return { member: m, loans: myLoans, counts, pmCount, total: openLoans.length, dailyMap };
  });
  ...
  return (
    <div style={styles.container}>
          ...
          <div style={styles.legend}>
            {ALL_GROUPS.map((g) => (
              <span key={g} style={styles.legendItem}>
                <span style={{ ...styles.legendDot, background: GROUP_COLORS[g].badge }} />
                {g}
              </span>
            ))}
          </div>
          <label style={styles.reloadBtn}>
            ↻ Reload File
            <input type="file" accept=".xlsx,.xls,.csv" onChange={handleFileInput} style={{ display: "none" }} />
          </label>
        </div>
      ...
      <div style={styles.grid}>
        {memberData.map(({ member, counts, pmCount, total, dailyMap, loans: memberLoans }) => {
          const isExpanded = expandedMember === member;
          const allCounts = { ...counts, PM: pmCount };
          return (
            <div key={member} style={styles.card}>
              <div
              ...
              >
                <div style={styles.memberRow}>
                  ...
                  <span style={styles.totalBadge}>{total} loans</span>
                </div>
              ...
              <div style={styles.groupRow}>
                {ALL_GROUPS.map((g) => (
                  <div key={g} style={{ ...styles.groupPill, background: GROUP_COLORS[g].bg, borderColor: GROUP_COLORS[g].border }}>
                    <span style={{ ...styles.groupCount, color: GROUP_COLORS[g].text }}>{allCounts[g]}</span>
                    <span style={{ ...styles.groupLabel, color: GROUP_COLORS[g].text }}>{g}</span>
                  </div>
                ))}
              </div>

              <div style={styles.weeksSection}>
                {weeks.map((weekDays, wi) => {
                  const weekTotal = weekDays.reduce((sum, d) => {
                    const dc = dailyMap[dateKey(d)];
                    return sum + ALL_GROUPS.reduce((s, g) => s + (dc?.[g] || 0), 0);
                  }, 0);
                  return (
                    <div key={wi} style={styles.weekBlock}>
                      <div style={styles.weekHeader}>
                        <span style={styles.weekLabel}>
                          {wi === 0 ? "This Week" : wi === 1 ? "Next Week" : "Week After"}
                        </span>
                        <span style={styles.weekRange}>{fmtWeekLabel(weekDays)}</span>
                        {weekTotal > 0 && <span style={styles.weekTotal}>{weekTotal}</span>}
                      </div>
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                ...(isPast && !isToday ? styles.pastCell : {}),
                              }}
                            >
                              <div style={{ ...styles.dayName, ...(isToday ? { color: "#1565C0", fontWeight: 700 } : {}) }}>
                                {DAY_NAMES[di]}
                              </div>
                                    ...
                                    (g) =>
                                      dc[g] > 0 && (
                                        <span
                                          ...
                                          style={{
                                            ...styles.dayPill,
                                            background: GROUP_COLORS[g].badge,
                                          }}
                                          title={g}
                                        >
                                          {dc[g]}
              ...
              {isExpanded && (
                <div style={styles.loanList}>
                  <div style={styles.loanListHeader}>All Loans</div>
                  {memberLoans.length === 0 ? (
                    ...
                    memberLoans
                      ...
                      .map((l, i) => (
                        <div key={i} style={styles.loanRow}>
                          <span style={{ ...styles.loanDot, background: GROUP_COLORS[l.group].badge }} />
                          <span style={styles.loanBorrower}>{l.borrower || "—"}</span>
                          <span style={styles.loanType}>{l.loanType}</span>
                          {l.pmOpen && l.checkInDate && (
                            <span style={{ ...styles.loanDate, color: GROUP_COLORS.PM.text }}>
                              ✓ {fmtDate(l.checkInDate)}
                            </span>
                          )}
                          <span style={styles.loanDate}>
                            {l.fundDate ? fmtDate(l.fundDate) : "No date"}
                          </span>
                          {l.pmOpen && <span style={{ ...styles.loanStatus, background: GROUP_COLORS.PM.bg, color: GROUP_COLORS.PM.text }}>PM</span>}
                          {l.status && <span style={styles.loanStatus}>{l.status}</span>}
                        </div>
                      ))
                  )}
                </div>
